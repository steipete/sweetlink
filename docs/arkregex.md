...
SweetLink now relies on [ArkRegex](https://www.npmjs.com/package/arkregex) to build every reusable pattern. The library gives us typed capture groups and consistent escaping. This document explains how to use the helpers, how we share patterns, and the guardrails that keep the regex surface maintainable.
...
* Define the pattern once at the module top level to avoid per-call allocations.
...
4. **Run** `pnpm lint` and `pnpm test -- --reporter=basic` to prove the migration.
...
| Put constants near the top of the module | Avoids repeated allocations and keeps patterns discoverable |
...
3. If lint still complains, document the exception next to the dynamic construction.
