 ...
 */
export async function withOpenClawFallback(operation, operationName) {
    try {
        return await operation();
    }
    catch (error) {
        const isConnectivity = error instanceof OpenClawError
            ? error.statusCode === 503 || error.statusCode === 409
            : isNetworkError(error);
        if (isConnectivity) {
            console.warn(`[sweetlink] OpenClaw unavailable for ${operationName}, falling back to native.`);
