...
export async function ensureDevStackRunning(targetUrl, options) {
    ...
    console.warn(`Dev stack did not become ready within ${Math.round(checkTimeout / 1000)}s. Expect follow-up commands to fail until the server finishes booting.`);
}
...
export async function isAppReachable(appBaseUrl, healthPaths) {
    ...
    const targets = new Set([appBaseUrl, ...additionalTargets]);
...
async function runCheckCommand(server, repoRoot) {
...
function launchStartCommand(server, repoRoot) {
