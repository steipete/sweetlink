...
class SweetLinkBrowserClient {
            ...
            const handleMessage = (event) => {
                try {
                    ...
                    if (payload.kind === 'command') {
                        void this.handleServerCommand(payload).catch((error) => {
    ...
    handleMetadataMessage(payload) {
        if (this.windowRef) {
            ...
            debugWindow.__sweetlinkMetadataEvents ?? (debugWindow.__sweetlinkMetadataEvents = []);
    ...
    scheduleAutoReconnect(reason) {
        ...
        const delayMs = Math.min(this.reconnectBaseDelayMs * 2 ** this.reconnectAttempts, 15000);
        const now = Date.now();
    ...
    announceStatus(status, detail = {}) {
        ...
        if (this.windowRef) {
            const clientWindow = this.windowRef;
            clientWindow.__sweetlinkStatusHistory ?? (clientWindow.__sweetlinkStatusHistory = []);
