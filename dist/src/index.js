...
program
    ...
    for (const session of sessions) {
        const heartbeatMsAgo = typeof session.heartbeatMsAgo === "number"
...
program
    ...
    const timeoutSeconds = typeof options.timeout === "number" && Number.isFinite(options.timeout)
...
program
    ...
    const hookOptions = {
        ...
        waitTimeout: typeof options.waitTimeout === "number" && Number.isFinite(options.waitTimeout)
            ? options.waitTimeout
            : undefined,
        delayMs: typeof options.delay === "number" && Number.isFinite(options.delay)
            ? options.delay
            : undefined,
    ...
    const payload = {
        ...
        selector: options.selector,
        quality,
        timeoutMs: typeof options.timeout === "number" && !Number.isNaN(options.timeout)
            ? options.timeout
            : 30_000,
    ...
    if (!handshake && context.devtoolsUrl) {
        ...
        if (!handshake) {
            ...
            if (recovered) {
                ...
                const recoveredUrl = typeof refreshed?.url === "string" && refreshed.url.length > 0
                    ? refreshed.url
                    : targetUrl.toString();
                handshake = {
                    sessionId: activeSessionId,
