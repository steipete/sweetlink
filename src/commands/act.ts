...
function buildTypeAction(options: ActCommandOptions, timeoutMs: number | undefined): OpenClawAction | null {
  const ref = validateStringLength(options.ref, MAX_REF_LENGTH);
  if (!ref || options.text === undefined) return null;
...
function buildSelectAction(options: ActCommandOptions, timeoutMs: number | undefined): OpenClawAction | null {
  const ref = validateStringLength(options.ref, MAX_REF_LENGTH);
  if (!(ref && options.values)) return null;
  ...
  return { kind: 'select', ref, values: validatedValues as string[], ...(timeoutMs !== undefined ? { timeoutMs } : {}) };
}
