...
export async function primeControlledChromeCookies(
  ...
  if (cookies.length === 0) {
    console.log(
      "No Chrome cookies found for this origin; continuing without priming the controlled window.",
    );
    return;
  }
  ...
  try {
    ...
    if (cookieNames.length > 0) {
      ...
      if (authCookies.length > 0) {
        ...
        console.log(
          "No obvious auth/session cookies detected—expect to re-authenticate in the controlled window.",
        );
      }
...
async function verifyCookieSync(
  ...
  try {
    ...
    if (missing.length > 0) {
      ...
      console.warn(
        `Warning: the controlled window is missing ${missing.length} cookie(s) (${missingNames}).`,
      );
    }
  } catch (error) {
