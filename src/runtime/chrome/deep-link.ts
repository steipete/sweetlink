...
export async function ensureDeepLinkAuthFlow(params: {
  ...
  let oauthAttempt: TwitterOauthAutoAcceptResult | undefined;
  ...
  try {
    ...
    if (devLoginUrl) {
      const devLoginResult = await attemptDevBootstrapLogin(
        page,
        devLoginUrl,
        targetUrl.toString(),
      );
    ...
    const hasOauthPage =
      currentUrl && isLikelyOauthUrl(currentUrl) ? true : await hasOauthTab(browser);
    ...
    return {
      ...
      navigatedToTarget: Boolean(
        currentUrl && urlsRoughlyMatch(currentUrl.toString(), targetUrl.toString()),
      ),
...
async function attemptOauthKickoff(page: PuppeteerPage, callbackUrl: string): Promise<boolean> {
...
function isLikelySignInUrl(url: URL): boolean {
  if (!url.pathname) {
    return false;
  }
...
function isLikelyOauthUrl(url: URL): boolean {
...
async function findMatchingPageUrl(browser: PuppeteerBrowser, targetUrl: string) {
  const pages = await browser.pages();
...
async function hasOauthTab(browser: PuppeteerBrowser): Promise<boolean> {
  const pages = await browser.pages();
  for (const page of pages) {
