...
export function maybeInstallMkcertDispatcher(): void {
  ...
  const mkcertRoot =
    cliEnv.caRoot ?? path.join(os.homedir(), "Library", "Application Support", "mkcert");
  const candidates = [
    overridePath,
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      console.warn(
        `Failed to register SweetLink TLS CA from ${formatPathForDisplay(candidate)}:`,
        error,
      );
    }
  }
