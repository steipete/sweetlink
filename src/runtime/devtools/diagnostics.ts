...
import type { BootstrapDiagnosticError, SweetLinkBootstrapDiagnostics } from "./types.js";
...
const appendAuthDiagnostics = (
  ...
  append(
    "  Hint: complete the real OAuth consent flow or issue a new session token before rerunning SweetLink.",
  );
};
...
const appendConsoleErrors = (
  ...
  for (const error of diagnostics) {
    ...
    append(
      `${label} console ${error?.type ?? "error"}${origin}: ${error?.message ?? "unknown error"}`,
    );
    if (error?.stack) {
...
export function logBootstrapDiagnostics(
  label: string,
  diagnostics: SweetLinkBootstrapDiagnostics,
): void {
  ...
  const errors: BootstrapDiagnosticError[] = Array.isArray(diagnostics.errors)
    ? diagnostics.errors
    : [];
...
export function logDevtoolsConsoleSummary(
  label: string,
  entries: readonly DevToolsConsoleEntry[],
  limit = 20,
): void {
  if (entries.length === 0) {
...
export function diagnosticsContainBlockingIssues(
  diagnostics: SweetLinkBootstrapDiagnostics,
): boolean {
