...
export async function attemptTwitterOauthAutoAccept({
  ...
  const context: SweetLinkOauthAuthorizeContext = {
    ...
    evaluateInDevToolsTab: async (targetUrl, expression) =>
      evaluateInDevToolsTab(devtoolsUrl, targetUrl, expression),
    urlsRoughlyMatch,
    connectPuppeteer: async (attempts = 3) => {
      try {
        const puppeteerModule: typeof import("puppeteer-core") = await import("puppeteer-core");
...
async function loadOauthAutomation(
  scriptPath: string | null,
): Promise<SweetLinkOauthAutomation | null> {
  if (!scriptPath) {
    return null;
  }
  ...
  if (value && typeof value === "object") {
    ...
    if (typeof record.handled === "boolean") {
      return {
        ...
        clickedText:
          typeof record.clickedText === "string" || record.clickedText === null
            ? record.clickedText
            : undefined,
