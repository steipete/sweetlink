...
const { collectChromeCookies, collectChromeCookiesForDomains } =
  await import("../../src/runtime/cookies");
...
describe("collectChromeCookies", () => {
  it("rehomes cookies for localhost targets and prunes incompatible entries", async () => {
    ...
    expect(call?.origins ?? []).toEqual(
      expect.arrayContaining(["https://localhost:4455", "https://example-auth.dev"]),
    );
  });
