...
describe("runtime/url utilities", () => {
  ...
  it("compares URLs loosely, allowing marketing suffixes", () => {
    expect(
      urlsRoughlyMatch("https://localhost:3000/timeline/index", "https://localhost:3000/timeline/"),
    ).toBe(true);
    expect(
      urlsRoughlyMatch(
        "https://localhost:3000/timeline/overview",
        "https://localhost:3000/timeline/",
      ),
    ).toBe(true);
    expect(
      urlsRoughlyMatch(
        "https://localhost:3000/settings/account",
        "https://localhost:3000/insights",
      ),
    ).toBe(false);
  ...
  it("honors configured redirects when comparing URLs", () => {
    ...
    expect(urlsRoughlyMatch("https://localhost:3000/", "https://localhost:3000/timeline")).toBe(
      true,
    );
    expect(
      urlsRoughlyMatch(
        "https://localhost:3000/workspace",
        "https://localhost:3000/workspace/overview",
      ),
    ).toBe(true);
